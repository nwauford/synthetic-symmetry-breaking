...
if strcmp(currpath(end-4:end), 'rep 3')
    ...
    data_path = [currpath '/Fig 2 rep 3/'];
end
...
if refit_gmm
    ...
    figname = 'rep3_3D_GMM_dists.png';
    saveas(fig,figname,'png')
    title('\fontsize{20}Replicate 3')

    azimuth = linspace(163, -197, 100);
    ViewZ = [azimuth; ones(1, 100).*10]';
    CaptureFigVid(ViewZ, 'rep3_Rotating_3D_GMM_Dists')
    
    % create arrays for saving GMM fits
    b_only_wells_r3 = ones(50000, 3, 64) .* NaN;
    y_only_wells_r3 = ones(50000, 3, 64) .* NaN;
    r_only_wells_r3 = ones(50000, 3, 64) .* NaN;
    b_only_subsets_r3 = ones(2820, 3, 64) .* NaN;
    y_only_subsets_r3 = ones(2820, 3, 64) .* NaN;
    r_only_subsets_r3 = ones(2820, 3, 64) .* NaN;


else
    load('b_only_wells_r3.mat')
    load('y_only_wells_r3.mat')
    load('r_only_wells_r3.mat')
    load('b_only_subsets_r3.mat')
    load('y_only_subsets_r3.mat')
    load('r_only_subsets_r3.mat')
end
    ...
    if refit_gmm
        for s = 1:length(myFiles)
            if str2num(myFiles(s).name(end-5:end-4)) == i
               ...
               b_only_wells_r3(1:length(b_only), :, i) = b_only;
               y_only_wells_r3(1:length(y_only), :, i) = y_only;
               r_only_wells_r3(1:length(r_only), :, i) = r_only;
               
            end
        end
    else
                
                b_only = b_only_wells_r3(:, :, i);
                b_only = b_only(~isnan(b_only));
                dim1 = length(b_only) / 3;
                b_only = reshape(b_only, [dim1, 3]);
                
                y_only = y_only_wells_r3(:, :, i);
                y_only = y_only(~isnan(y_only));
                dim1 = length(y_only) / 3;
                y_only = reshape(y_only, [dim1, 3]);
                
                r_only = r_only_wells_r3(:, :, i);
           ...
           % plot same number of cells in each subplot
            ...
            if refit_gmm
               ...
               b_only_subsets_r3(1:length(b_only_subset), :, i) = b_only_subset;
               y_only_subsets_r3(1:length(y_only_subset), :, i) = y_only_subset;
               r_only_subsets_r3(1:length(r_only_subset), :, i) = r_only_subset;

               
            else
                b_only_subset = b_only_subsets_r3(:, :, i);
                b_only_subset = b_only_subset(~isnan(b_only_subset));
                dim1 = length(b_only_subset) / 3;
                b_only_subset = reshape(b_only_subset, [dim1, 3]);
                
                y_only_subset = y_only_subsets_r3(:, :, i);
                y_only_subset = y_only_subset(~isnan(y_only_subset));
                dim1 = length(y_only_subset) / 3;
                y_only_subset = reshape(y_only_subset, [dim1, 3]); 
                
                r_only_subset = r_only_subsets_r3(:, :, i);
...
%  error()
 ...
 if refit_gmm
     save('b_only_wells_r3.mat', 'b_only_wells_r3')
     save('y_only_wells_r3.mat', 'y_only_wells_r3')
     save('r_only_wells_r3.mat', 'r_only_wells_r3')
     save('b_only_subsets_r3.mat', 'b_only_subsets_r3')
     save('y_only_subsets_r3.mat', 'y_only_subsets_r3')
     save('r_only_subsets_r3.mat', 'r_only_subsets_r3')
 end
 
%  min_size
 
  figname = 'rep3_r_vs_y.png';
saveas(fig_ry,figname,'png')

  figname = 'rep3_b_vs_y.png';
saveas(fig_by,figname,'png')

  figname = 'rep3_b_vs_r.png';
...
fig=figure;
 ...
 title('Fraction R+ Y- B- Replicate 3')
 colormap viridis
 hm.CellLabelFormat = '%0.0f';
 hm.FontSize = hmfontsize;
 colorbar
 figname = 'rep3_red_only_heatmap.png';
 hm.GridVisible = 'off';
 caxis([0, 100]);
saveas(fig,figname,'png')
 ...
 hm = heatmap(frac_y_only .* 100, 'CellLabelColor','white');
  title('Fraction Y+ R- B- Replicate 3')
  colormap viridis
   hm.CellLabelFormat = '%0.0f';
   hm.FontSize = hmfontsize;
   hm.GridVisible = 'off';
   caxis([0, 100]);
  colorbar
   figname = 'rep3_yellow_only_heatmap.png';
saveas(fig,figname,'png')
  
   fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_b_only .* 100, 'CellLabelColor','white');
  title('Fraction B+ R- Y- Replicate 3')
  colormap viridis
  colorbar
   hm.CellLabelFormat = '%0.0f';
   hm.FontSize = hmfontsize;
    hm.GridVisible = 'off';
    caxis([0, 100]);
   figname = 'rep3_blue_only_heatmap.png';
saveas(fig,figname,'png')

save('rep3_r_frac.mat', 'frac_r_only')
save('rep3_b_frac.mat', 'frac_b_only')
save('rep3_y_frac.mat', 'frac_y_only')
