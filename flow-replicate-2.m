function out = func_mCE40m4_13_all_GMM

usrC = strsplit(pwd,'/');

flow_analysis_code_path = ['/Users/' usrC{3} '/Documents/MIT/Weiss Lab/Flow/Analysis/release 3 pipeline'];

data_path = ['/Users/' usrC{3} '/Documents/MIT/Weiss Lab/Flow/mCE40m4_013/'];
...
%  min_size
  ...
  figname = 'exp13_r_vs_y.png';
saveas(fig_ry,figname,'png')

  figname = 'exp13_b_vs_y.png';
saveas(fig_by,figname,'png')

  figname = 'exp13_b_vs_r.png';
saveas(fig_br,figname,'png')
 
fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_r_only .* 100, 'CellLabelColor','white');
 title('Fraction R+ Y- B- Exp 13')
 colormap viridis
 hm.CellLabelFormat = '%0.0f';
   hm.GridVisible = 'off';
        caxis([0, 100]);
 colorbar
 figname = 'exp13_red_only_heatmap.png';
saveas(fig,figname,'png')
 
 fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_y_only .* 100, 'CellLabelColor','white');
  title('Fraction Y+ R- B- Exp 13')
  colormap viridis
   hm.CellLabelFormat = '%0.0f';
     hm.GridVisible = 'off';
     caxis([0, 100]);
  colorbar
   figname = 'exp13_yellow_only_heatmap.png';
saveas(fig,figname,'png')
  
   fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_b_only .* 100, 'CellLabelColor','white');
  title('Fraction B+ R- Y- Exp 13')
  colormap viridis
  colorbar
   hm.CellLabelFormat = '%0.0f';
     hm.GridVisible = 'off';
     caxis([0, 100]);
   figname = 'exp13_blue_only_heatmap.png';
saveas(fig,figname,'png')

save('exp13_r_frac.mat', 'frac_r_only')
save('exp13_b_frac.mat', 'frac_b_only')
save('exp13_y_frac.mat', 'frac_y_only')
