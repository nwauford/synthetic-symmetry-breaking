...
if strcmp(currpath(end-4:end), 'rep 1')
    ...
    data_path = [currpath '/Fig 1D, SI Fig 2-3 rep 1/'];
end
...
fracs_rep1 = zeros(7, 4); % bin; [f_none f_r f_y f_both]
            ...
            lo_b = t_comp(i_lo_b & i_hi_b, :);
                ...
                total_both = 0;
                   ...
                   i_r = and(lo_b(:, chR) > r_thresh, lo_b(:, chY) < y_thresh);
                    ...
                    title(['Replicate 1 R vs Y in EBFP- cells condition ' t(9)])
                    filename = ['Replicate 1 R vs Y in EBFP- cells condition ' t(9)];
                     ...
                     fracs_rep1(str2num(t(9)), :) = [f_none f_r f_y f_both];
                    ...
                    title(['Replicate 1 B vs SSC condition ' t(9)])
                    filename = ['Replicate 1 B vs SSC condition ' t(9)];
                     saveas(fig, filename, 'png');
...
save('fracs_rep1.mat', 'fracs_rep1')
