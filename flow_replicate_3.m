function out = func_mCE40m4_14_all_GMM

usrC = strsplit(pwd,'/');

flow_analysis_code_path = ['/Users/' usrC{3} '/Documents/MIT/Weiss Lab/Flow/Analysis/release 3 pipeline'];

data_path = ['/Users/' usrC{3} '/Documents/MIT/Weiss Lab/Flow/mCE40m4_014/'];
...
% min_size = Inf;
 ...
 for i = 1:64
    ...
    %            end
          ...
          if refit_gmm & (i == 1 || i == 57 || i == 30 || i == 64 || i ==8)
...
%  min_size
  ...
  figname = 'exp14_r_vs_y.png';
saveas(fig_ry,figname,'png')

  figname = 'exp14_b_vs_y.png';
saveas(fig_by,figname,'png')

  figname = 'exp14_b_vs_r.png';
saveas(fig_br,figname,'png')
 
fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_r_only .* 100, 'CellLabelColor','white');
 title('Fraction R+ Y- B- Exp 14')
 colormap viridis
 hm.CellLabelFormat = '%0.0f';
hm.GridVisible = 'off';
caxis([0, 100]);
 colorbar
 figname = 'exp14_red_only_heatmap.png';
saveas(fig,figname,'png')
 
 fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_y_only .* 100, 'CellLabelColor','white');
  title('Fraction Y+ R- B- Exp 14')
  colormap viridis
   hm.CellLabelFormat = '%0.0f';
   hm.GridVisible = 'off';
   caxis([0, 100]);
  colorbar
   figname = 'exp14_yellow_only_heatmap.png';
saveas(fig,figname,'png')
  
   fig=figure;
 set(fig,'color','w');
 hm = heatmap(frac_b_only .* 100, 'CellLabelColor','white');
  title('Fraction B+ R- Y- Exp 14')
  colormap viridis
  colorbar
   hm.CellLabelFormat = '%0.0f';
   caxis([0, 100]);
    hm.GridVisible = 'off';
   figname = 'exp14_blue_only_heatmap.png';
saveas(fig,figname,'png')

save('exp14_r_frac.mat', 'frac_r_only')
save('exp14_b_frac.mat', 'frac_b_only')
save('exp14_y_frac.mat', 'frac_y_only')
