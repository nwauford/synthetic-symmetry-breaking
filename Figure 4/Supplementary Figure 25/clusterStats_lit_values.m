...
if ~exist('data_lit_values.mat')
        out_y = clust_stats(y_param, num_doses, area_thresh, dirs);
        out_r = clust_stats(r_param, num_doses, area_thresh, dirs);
        data_lit_values = {out_y, out_r};
        save('data_lit_values','data_lit_values')
else 
    load('data_lit_values.mat')
    out_y = data{1};
    out_r = data{2};
end
