...
dirs = {'analysis_images/rep1/', 'analysis_images/rep2/', 'analysis_images/rep3/'};

if ~exist('data_no_transitions.mat')
        out_y = clust_stats(y_param, num_doses, area_thresh, dirs);
        out_r = clust_stats(r_param, num_doses, area_thresh, dirs);
        data_no_transitions = {out_y, out_r};
        save('data_no_transitions','data_no_transitions')
else 
    load('data_no_transitions.mat')
    out_y = data{1};
    out_r = data{2};
end
...
numClustArray = out_y{1};
    ...
    title(t, ['Yellow no cell state transitions'])
    ...
    saveas(t, "Y-heatmaps-no_transitions.png");
    ...
    fig = figure('Position', [525 1 563 804]);
    t = tiledlayout(2,1,'TileSpacing','Compact');
    title(t, ['Red no cell state transitions'])
    ...
    saveas(t, "R-heatmaps-no_transitions.png");
